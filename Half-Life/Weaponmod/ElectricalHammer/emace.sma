...
public Knife_Deploy(const iItem,
  const iPlayer,
    const iClip) {
...
public Knife_PrimaryAttack(const iItem,
  const iPlayer) {
...
FindHullIntersection(const Float: vecSrc[3], & iTrace,
...
public Knife_SwingAgain(const iItem,
  const iPlayer) {
