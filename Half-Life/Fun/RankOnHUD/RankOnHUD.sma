...
public plugin_init()
{
	register_plugin("HL HUD Ranks", "1.0", "Glaster");
	...
	WriteTop15();
	set_task(REFRESH_RATE, "SayRank", 2, .flags = "b");
...
public SayRank()
{
	...
	for (new i = 0; i < iNum; i++)
	{
		...
		set_hudmessage(255, 255, 0, 0.0, 0, 9, .holdtime = (REFRESH_RATE + 1.2), .fadeintime = 0.1, .fadeouttime = 0.1, .channel = 3);
		show_hudmessage(iPlayer, "*Your rank is %d of %d with %d kills and %d deaths.", Position, ArraySize(g_SteamID), g_Data[iPlayer][Data_Kills], g_Data[iPlayer][Data_Deaths]);
	}
}
...
public SortRanks(Array: SteamID, Position1, Position2)
{
	...
	new kpd1 = Kills1 - Deaths1;
	new kpd2 = Kills2 - Deaths2;
	if (kpd1 <= kpd2)
		return 1;

	if (kpd1 > kpd2) return -1;
...
{#
	...
	static
	const Header[] = "<body bgcolor=#000000 > < font color=#FFB000><pre>%5s  %22s  %5s  %5s^n^n";
	static
	const Buffer[] = "%4d   %22s  %5d  %6d^n";#
	else
		static
	const Header[] = "%5s  %22s  %5s  %5s^n^n";
	static
	const Buffer[] = "%5d   %22s  %5d  %5d^n";#
	endif
